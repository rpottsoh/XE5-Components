...
type
  ...
  {$ENDIF}
    ...
    public
        ...
        Procedure DMAFlush;
        Procedure Trace(BitMask : integer; Str : TString);
        Procedure TraceValue(BitMask : integer; Str : TString; Value : integer);
        Procedure TraceShadow(Bitmask : integer; Str : TString; shadow : pointer);
        ...
        Procedure TimerSetup(TimerInterval : integer; TimerType : boolean);
        ...
        Function DimStr(var S : TString; size : integer) : pointer;
        Function GlobalDim(name:TString;wordsize, wordcount : integer; var data) : pointer;
        Function ExternDim(name:TString;wordsize, wordcount : integer; var data) : pointer;
        ...
        Procedure _Else;
        Procedure _Endif;
        Procedure _Goto(const alabel : TString);
        Procedure _Label(const alabel : TString);
    ...
    private
        ...
        Procedure AddName(GlobalOrExtern: integer; const name: TString);
...
var
     ...
     p: pointer;        // temporary place for TString pointer
begin
     ...
     with flist do begin
        ...
        while ii < count do begin
            ...
            with buff^[ii] do begin
                    // replace the Dim Global and extern TString pointer with the actual values
                if (WINRT_COMMAND = WINRT_GLOBAL) or (WINRT_COMMAND = WINRT_EXTERN) then begin
                        // copy the content of the TString
                    len := length(TString(value));
                    extraitems := port;
                    p := pointer(value);
                    system.move(p^, value, len + 1);
                    // decrement TString refcount
                    TString(p) := '';
                    port := len;
                    inc(ii, extraitems);
                    end;
...
begin
     ...
     with flist do begin
          ...
          if command = btWaitInterrupt then begin
                ...
                // send the buffer down, set up the timer routine
          else if command = btSetTimer then begin
                if not WinRTSetTimer(fhandle, buff^, buffsize, Length) then begin
                     lasterr := GetLastError;
                     raise eWinRTError.createfmt('Error in WinRTSetTimer Device %d Error %d', [fDeviceNo, LastErr])
                     end
                end
...
begin
    AddItem(DMA_FLUSH, 0, 0);
     ...
     //          Str - the TString to trace
     // Outputs: none
Procedure tWinRT.Trace(BitMask : integer; Str : TString);

//  procedure CpyStr(Var Dest : PANSIChar; Var Source :
var
     Param1,
     Param2,
     Param3,
     ii    : integer;
     Count : Double;
     Token : array[0..3] of TChar;
begin
          // add the trace command to the command list
     AddItem(WINRT_TRACE, 0, BitMask shl 16 or Length(Str));
     Count := Length(Str);
     ii := 0;
          // add the TString to trace to the command list
     while ii < Count do begin
          StrLCopy(TPChar(@Token[0]),TPChar(@Str[1+ii]), 4);
    ...
    //      buffer, which can then be retrieved using the Debug Trace option
     ...
     // Outputs: none
Procedure tWinRT.TraceValue(BitMask : integer; Str : TString; Value : integer);
var
     Param1,
     Param2,
     Param3,
...
begin
     ...
     Count := Length(Str);
     ii := 0;
          // add the TString to the command list
     while ii < Count do begin
          StrLCopy(TPChar(@Token[0]), TPChar(@Str[1+ii]), 4);
          Param1 := integer(Token);
          ii := ii + 4;
     ...
     // Outputs: none
Procedure tWinRT.TraceShadow(Bitmask : integer; Str : TString; Shadow : pointer);
var
     Param1,
     Param2,
     Param3,
...
begin
    ...
    // tWinRT.TimerSetup - Sets up a section of code to be executed
    //                     on a repeating or one-shot timer.
    //
    //                     One-shot timer code is execute only once
    //                     at the end of the first timer interval.
    //                     Use TimerStart() to start the timer.  A
    //                     one-shot timer is stopped automatically 
    //                     after the first interval.
    //
    //                     Repeating timer code is executed each timer
    //                     interval until TimerStop() is called.  Use
    //                     TimerStart() to start the timer.
    //
    // Inputs:  TimerInterval - timer interval in milliseconds
    //          TimerType - true indicates a repeating interval timer,
    //                      false indicates a one-shot timer
    // Outputs: none
Procedure tWinRT.TimerSetup(TimerInterval : integer; TimerType : boolean);
begin
    AddItem(SETUP_TIMER, TimerInterval, integer(TimerType));
    end;

    // tWinRT.TimerStart - Starts a one shot or repeating timer that
    //                     was set up with a previous call to TimerSetup.
    // Inputs:  none
    // Outputs: none
Procedure tWinRT.TimerStart;
begin
    AddItem(START_TIMER, 0, 0);
    end;

    // tWinRT.TimerStop - Stops a repeating timer or aborts a one shot
    //                    timer that was set up with a previous call 
    //                    to TimerSetup and started with a previous
    //                    call to TimerStart
    // Inputs:  none
    // Outputs: none
Procedure tWinRT.TimerStop;
begin
    AddItem(STOP_TIMER, 0, 0);
    end;
...
Procedure tWinRT.SetTimer;
begin
    ProcessList(btSetTimer)
    end;    { SetTimer }
...
begin
     ...
     //  tWinRT.DimStr - Declare a shadow variable to a huge TString
     //                  This function is needed due to the special
     //                  way huge strings are processed in Delphi 2
     // Inputs
     //      S:      The pascal huge TString being shadowed
     //      size:   The maximum allowed size for S. Should include space
     //              for an extra eos character which is #0, like an ASCIZ TString
     ...
     //      bConst: Is the TString a constant, rather than a variable.
     // Outputs:     returns a pseudo pointer to the shadow variable created. This is
     //              not a valid pascal pointer and should not be used as such
Function tWinRT.DimStr(var S : TString; size : integer) : pointer;
var
     index,
     extrabytes,
     extraitems  : integer;
...
begin
    ...
    //                  label
Procedure tWinRT._Goto(const aLabel : TString);
var
    position,
    index,
    jumpofs : integer;
...
Procedure tWinRT._Label(const aLabel : TString);
var
     controlitemindex,
     index   : integer;
     Jumpp           : pCONTROL_ITEM_ex;
...
begin
     ...
     // Outputs:     returns a pseudo pointer to the shadow variable created. This is
     //              not a valid pascal pointer and should not be used as such
function tWinRT.ExternDim(name: TString; wordsize, wordcount: integer;
  var data): pointer;
var
     i,
     flags,
...
begin
     ...
     // Outputs:     returns a pseudo pointer to the shadow variable created. This is
     //              not a valid pascal pointer and should not be used as such
function tWinRT.GlobalDim(name: TString; wordsize, wordcount: integer;
  var data): pointer;
var
     i,
     flags,
...
procedure tWinRT.AddName(GlobalOrExtern: integer; const name: TString);
var
     index,
     extrabytes,
     extraitems:    integer;
